...
def turn_text(window, font, player, game_winner, color=(255, 255, 255)):
    sx = 435 // 2 - 35
    sy = 590
    pygame.draw.rect(window, (0, 0, 0), (sx, sy, 300, 50))
    if not game_winner:
        if player == 'cross':
            label = font.render('X\'s turn', 1, color)
        else:
            label = font.render('O\'s turn', 1, color)
...
def statistic(window, record, color=(255, 255, 255)):
    ...
    label_X = font.render('Player 1:        ' + str(record[0]), 1, color)
    label_draw = font.render('Draw:             ' + str(record[1]), 1, color)
    label_O = font.render('Player 2:        ' + str(record[2]), 1, color)
...
if __name__ == '__main__':
    ...
    while run:
        ...
        turn_text(game_window, game_font, current_object, winner)
